	...
	$.ajax({
		...
		complete : function(){
			...
			if(url.includes("admin/security") ){
				$("<script/>", {
			        src: `${cPath}/resources/myJS/kch/publicAddress.js`,
			        type: "text/javascript"
			    }).appendTo("body");
			}
		}
...
function innerBodyAjax(url,inner,data){
	...
	$.ajax({
		...
		complete : function(){
			...
			if(url.includes("company/insert") ){
				$("<script/>", {
...
$(document).on('ajaxComplete', function(event, jqXHR, settings){
	...
	if(settings.url == "http://localhost/WareBridge/login") return false;
	...
	if(settings.url.indexOf('/user/book/') != -1){
		settings.url = 'http://localhost/WareBridge/user/book/bookSub'
	}
